...
def _pick_template(topic: str, difficulty: str, grade: int) -> Optional[str]:
    if topic not in TEMPLATES or difficulty not in TEMPLATES[topic]:
        return None
    band = TEMPLATES[topic][difficulty]
...
def generate_variation_from_template(template: str, grade: int) -> str:
    """Replace each NUM token with grade-scaled numbers; keep structure stable."""
    # Determine numeric range by grade band
    if grade <= 5:
        low, high = 5, 40
    elif grade <= 9:
        low, high = 12, 120
...
def generate_question(grade: int, difficulty: str, topic: str, model: str = "phi", force_ai: bool = False) -> Tuple[str, str, str, List[str]]:
    ...
    if attempt_ai:
        ...
        if raw:
            ...
            if len(cleaned) >= 8:
                print("[question-gen] SOURCE: LLM")
                return cleaned, "", "", []
        # If AI path failed or yielded unusable text, fall back to template below

    if template:
        question = generate_variation_from_template(template, grade)
        print("[question-gen] SOURCE: TEMPLATE")
    else:
        # ultimate fallback static
        question = generate_variation_from_template("Solve for x: 3x + 7 = 22", grade)
        print("[question-gen] SOURCE: FALLBACK")
    return question, "", "", []
...
def get_generic_hint(topic: str, question: str) -> str:
    base = {
        ...
        "trigonometry": "Use fundamental trig identities.",
        "statistics": "Identify what measure is being asked (mean, median, etc.).",
    }
